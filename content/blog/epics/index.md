...
PVA is closely related to the Channel Access (CA) protocol (v3 EPICS), which may work alongside, and is intended to supersede (for new projects it is recommended to use PVA). Fundamentally PVA is similar to CA (name search with UDP + data transfer with TCP), but supports arbitrary data structures instead of DBR\_\* types. Also allows RPC operations!
