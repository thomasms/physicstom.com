...
class Refresher extends React.Component {
  ...
  componentDidMount() {
    ...
    this.setState(
      ...
      function () {
        cookies.set("count", this.state.count, { path: "/" })
      }
    )
  }
  ...
  render() {
    var hidden =
      this.state.count > COUNT_LIMIT ? (
        <div>You are in!</div>
      ) : (
        <div>
          <p>Refresh me!</p>
          <p>Number of hits: {this.state.count}</p>
        </div>
      )

    return (
      <div className="apps-refresher-main">
        <header className="App-header">{hidden}</header>
        <button className="button is-danger is-rounded" onClick={this.reset}>
          Reset
        </button>
      </div>
    )
